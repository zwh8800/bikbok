...
├── player.js     # 播放器管理 (load / preload / swap / setup / speed)
├── ui.js         # 覆盖层创建 + UI 更新 (createOverlay / updateUI / showMessage)
...
| 预加载 | player.js | `preloadVideo()`, `swapAndPlayPreloaded()`, `handlePreloadLoaded()` |
...
- 新增按键需要同步更新 `player.js` 中两处 `navKeys` 数组（`setupPlayerInIframe` 和 `swapAndPlayPreloaded`）
