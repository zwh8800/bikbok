 ...
 *
 * 处理所有用户输入事件：
 *   - onKeyDown(): 键盘事件（↑↓ 导航 / ← → 进度 / F 全屏 / I O 倍速 / B 退出 / Escape 关闭）
...
(function (api) {
   ...
   *   2. F / Shift+F — 切换浏览器全屏
   *   3. I — 降低倍速（-0.25x，范围 0.25-3.0x）
   *   4. O — 提高倍速（+0.25x，范围 0.25-3.0x）
   *   5. B — 全局切换（退出 TikTok 模式）
   *   6. ← → — 快进快退（透传给 B 站播放器，延迟 100ms 读取进度后显示进度指示器）
   *   7. Space — 播放/暂停（透传，不拦截）
   *   8. ↑ — 上一个视频（300ms 防抖）
   *   9. ↓ — 下一个视频（300ms 防抖）
   *
